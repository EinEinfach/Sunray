...
//#pefine ENABLE_PASS   1        // comment out to disable password authentication
...
#define WHEEL_BASE_CM         44         // wheel-to-wheel distance (cm)        
...
#define TICKS_PER_REVOLUTION  340     // odometry ticks per wheel revolution (RM24)
...
#define MOTOR_PID_LP     1.0    // encoder low-pass filter (use for low encoder tickcount - use zero to disable)
#define MOTOR_PID_KP     0.01   // 0.25 if using sunray pid, if using pico pid set it to 0.0
...
#define BUMPER_TRIGGER_DELAY  300		// bumper must be active for (ms) to trigger
