...
void Motor::speedPWM ( int pwmLeft, int pwmRight, int pwmMow, int speedRight, int speedLeft )
{
  ...
  motorDriver.setMotorPwm(pwmLeft, pwmRight, pwmMow, releaseBrakes, speedRight, speedLeft);
}
...
void Motor::setLinearAngularSpeed(float linear, float angular, bool useLinearRamp){
   ...
   // RPM = V / (2*PI*r) * 60
   motorRightRpmSet =  rSpeedForRpm / (PI*(((float)wheelDiameter)/1000.0)) * 60.0;   
   motorLeftRpmSet = lSpeedForRpm / (PI*(((float)wheelDiameter)/1000.0)) * 60.0;   
